...
import { initCanvas, drawStarfield, drawTitleScreen, drawCountdown, drawPauseScreen, 
         ...
         flashMessage, drawFlashMessage, drawOrientationWarning } from './ui.js'; 
...
import { bgImage, bgLoaded } from './assets.js';
...
ctx.translate(cameraShake.offsetX, cameraShake.offsetY);
  ...
  if (drawOrientationWarning(ctx)) {
    ctx.restore();
    return; // Don't draw game if in portrait mode
  }
...
if (isGameOver) {
  drawGameOverOverlay(ctx, mouse, adState.blockerDetected, adState.failureMessage, 
